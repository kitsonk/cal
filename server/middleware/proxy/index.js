...
function setupHeaders(proxyReq, req, host) {
	/* Some hosts will error out if the host header is not set properly */
	proxyReq._headers.host = host || proxyReq._headers.host;
	if(!req.authInfo) {
		return;
	}
...
function createProxy(options) {
	...
	proxy.on('proxyReq', function(proxyReq, req, res, options) {
		setupHeaders(proxyReq, req, options.target.host);
...
// Parse the rules to get flags, replace and match pattern
	...
	return function reverseProxy(req, res, next) {
		...
		rules.some(function(rule) {
			...
			if (rule.proxy) {
				var options = {},
						target = url.parse(req.url.replace(rule.regexp, rule.replace));
				req.url = target.path;
